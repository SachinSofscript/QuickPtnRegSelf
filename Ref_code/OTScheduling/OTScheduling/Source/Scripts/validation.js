...
function ValidateNumWithNegativeDeci(sender) {
    if ((event.keyCode < 48) || (event.keyCode > 57)) {
		if (event.keyCode != 46) //Added By Sachin 20190610
		{
			//event.returnValue = false;
...
function ValidateNumWithNegativeDeci(sender) {
    if ((event.keyCode < 48) || (event.keyCode > 57)) {
		if (event.keyCode != 46 && event.keyCode != 45) //Added By Sachin 20190610
		{
			//event.returnValue = false;
			event.preventDefault();
			sender.focus();
...
function ValidateDt(sender) {
    ...
    if ((event.keyCode < 48) || (event.keyCode > 57)) {
      if (event.keyCode !=47 && event.keyCode != 13) { //Added By Sachin 20190610
        //event.returnValue = false;
        event.preventDefault();
        sender.focus();
      }
...
function ValidateTm(sender) {
    ...
    if ((event.keyCode < 48) || (event.keyCode > 57)) {
      if (event.keyCode != 58) { //Sachin 20191216
        //event.returnValue = false;
        event.preventDefault();
        sender.focus();
      }
...
function HelpImg_uc(e, me, cntrl, RequestTyp, StylesheetType, formName, extra) {
    var AppSubMdCd = document.getElementById("ContentPlaceHolder1_ucInsRegTPA_HfAppMdCd").value
    var AppMdCd = document.getElementById("ContentPlaceHolder1_ucInsRegTPA_HfAppMdCd").value
    evt = e || window.event;
    // if (evt.keyCode == 113 || evt.keyCode == 0 || evt.keyCode == undefined) {
    HelpDetails(e, me, cntrl, RequestTyp, StylesheetType, formName, extra)
    //}
...
function Help_uc(e, me, cntrl, RequestTyp, StylesheetType, formName, extra) {
    var AppSubMdCd = document.getElementById("ContentPlaceHolder1_ucInsRegTPA_HfAppMdCd").value
    var AppMdCd = document.getElementById("ContentPlaceHolder1_ucInsRegTPA_HfAppMdCd").value
    evt = e || window.event;
    if (evt.keyCode == 113 || evt.keyCode == 0) {
...
function HelpWithParamImg_uc(e, me, cntrl, RequestTyp, StylesheetType, formName, extra) {
    var AppSubMdCd = document.getElementById("ContentPlaceHolder1_ucInsRegTPA_HfAppSubMdCd").value;
    var AppMdCd = document.getElementById("ContentPlaceHolder1_ucInsRegTPA_HfAppMdCd").value;
    evt = e || window.event;
    //   if (evt.keyCode == 113 || evt.keyCode == 0 || evt.keyCode == undefined) {
    HelpDetailsWithParam_uc(e, me, cntrl, RequestTyp, StylesheetType, formName, extra)
    // }
}
...
/// Added by Amol 2019-07-01  // Only For WEB CAM in Patient Master Index
function HelpWithParamForCam(e, me, cntrl, RequestTyp, StylesheetType, formName, extra) {
    var AppSubMdCd = document.getElementById("ContentPlaceHolder1_HfAppSubMdCd").value;
    var AppMdCd = document.getElementById("ContentPlaceHolder1_HfAppMdCd").value;
    evt = e || window.event;
    if (evt.keyCode == 113 || evt.keyCode == 0 || evt.keyCode == undefined) {
...
function ReturnValues_uc(MyArgs, RequestTyp, cntrl, action) {
    if (MyArgs == '') {
        window.opener.ButtonClick(cntrl);
    }
    else {
        window.opener.SetReturnValue_uc(MyArgs, RequestTyp, cntrl, action);
    }
...
function SetReturnValue_uc(MyArgs, RequestTyp, cntrl, action) {
    //cntrl = cntrl.value.replace(/^\s+|\s+$/g, '');
    //var ArrayCntrl = cntrl.split(',');
    //for (i = 0; i <= ArrayCntrl.length - 1 ; i++) {
    //    document.getElementById("ContentPlaceHolder1_" + ArrayCntrl[i] + "").value = MyArgs[i];
    ...
    var IsButtonPresent = 'false';
    for (i = 0; i <= ArrayCntrl.length - 1 ; i++) {
        var x = document.getElementById("ContentPlaceHolder1_ucInsRegTPA_" + ArrayCntrl[i] + "");
        //  alert(x);
        if (document.getElementById("ContentPlaceHolder1_ucInsRegTPA_" + ArrayCntrl[i] + "").type == 'submit') {
            document.getElementById("ContentPlaceHolder1_ucInsRegTPA_" + ArrayCntrl[i] + "").click();
            IsButtonPresent = 'true';
        }
        else {
            document.getElementById("ContentPlaceHolder1_ucInsRegTPA_" + ArrayCntrl[i] + "").value = MyArgs[i];
        }
...
function callsettings(no) {
    ...
    if (no == '7') {
	 ...
	 if (no == '8') {
        settings = "toolbar=no," + "scrollbars=no," + "location=no," + "statusbar=no," + "menubar=no," + "resizable=0," + "width=610," + "height=650," + "left =388," + "top=8"
        return settings;
    }
	   if (no == '9') {
        settings = "toolbar=no," + "scrollbars=no," + "location=no," + "statusbar=no," + "menubar=no," + "resizable=0," + "width=1190," + "height=550," + "left =10," + "top=8"
        return settings;
    }
}
...
//Added For Batch should not contais single Quote (')
function FunBatchValidator(evt) {
            var str = $('#' + $(evt).attr('id').toString()).val();
            if (str.search(/'/g) >= 0) {
                str = str.replace(/'/g, '');
                $('#' + $(evt).attr('id').toString()).val(str)
                $($(evt).attr('id')).val(str);
                //evt.preventDefault();
                // alertify.set('notifier', 'position', 'top-right');
                alertify.error("Single Quote Not Allowed");
                return false;
            }
        }
		
		function batchvalidatoronsave(grid, batch) {
    var _grid = grid;
    var _batch = batch;
    $('[id$="' + _grid + '"] tr:has(td)').each(function (e) {
        if ($('[id$="' + _grid + '_' + _batch + '_' + e.toString() + '"]').attr('type') == "text") {
            var batchVAl = $('[id$="' + _grid + '_' + _batch + '_' + e.toString() + '"]').val();
        }
        else {
            var batchVAl = $('[id$="' + _grid + '_' + _batch + '_' + e.toString() + '"]').text();
        }

        // var isFreeItem = $('[id$="lblisfreeItem_' + e.toString() + '"]').text()
        if (batchVAl.search(/'/g) >= 0) {
            event.preventDefault();
            $('[id$="' + _grid + '_' + _batch + '_' + e.toString() + '"]').css('background-color', 'rgba(255, 152, 0, 0.42)');
            // alertify.set('notifier', 'position', 'top-right');
            alertify.error("Single Quote Present in batch Inside a grid Which not allowed.");
            return false;
        }
    })
}
	
function validate_email_uc(email) {
	var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
	var address = document.getElementById("ContentPlaceHolder1_ucInsRegTPA_" + email + "").value;

	if (reg.test(address) == false) {
		document.getElementById("ContentPlaceHolder1_ucInsRegTPA_" + email + "").value = "";
		setfocus('Invalid Email Address', email);
		return false;
	}
}

function validateTime_uc(time) {
	var d = new Date();
	var hrs = d.getHours();
	var min = d.getMinutes();
	...
	var t = document.getElementById("ContentPlaceHolder1_ucInsRegTPA_" + time + "").value; // document.getElementById("<%=txtAdmissionTm.ClientID%>").value;
	if (t.length == 0) {
		// alertify.alert("Enter Valid Time");
		setfocus('Enter Valid Time', time);
	}
	if (t.length > 5) {
		...
		var tm = hrs + ":" + min;
		//    document.getElementById("<%=txtAdmissionTm.ClientID%>").value = tm;
		document.getElementById("ContentPlaceHolder1_ucInsRegTPA_" + time + "").value = tm;
		setfocus('Invalid Time Format', time);
		return false;
	}
...
function hover_row(hover_grd) {
    $("#" + hover_grd + " tr:has(td)").hover(function () {
